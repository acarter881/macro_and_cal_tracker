...
export function CustomFoodTab() {
  ...
  function toGrams(amount: number, unit: LabelUnit, dens: number) {
    switch (unit) {
      ...
      case "fl oz":
        return amount * 29.5735 * dens;
      case "cup":
        return amount * 240 * dens;
      case "tbsp":
        return amount * 15 * dens;
      case "tsp":
        return amount * 5 * dens;
      default:
        return NaN;
    }
  }

  const watchedConverterFields = watch([
    "labelKcal",
    "labelP",
    "labelC",
    "labelF",
  ...
  const labelPer100 = useMemo(() => {
    const [labelKcal, labelP, labelC, labelF, servAmt, servUnit, density] =
      watchedConverterFields;
    const [_k, _p, _c, _f, _amt, _dens] = [
      labelKcal,
      labelP,
      labelC,
      labelF,
  ...
  function applyConverterValues() {
    ...
    setValue("kcal_per_100g", parseFloat(labelPer100.kcal.toFixed(2)), {
      shouldValidate: true,
    });
    setValue("protein_g_per_100g", parseFloat(labelPer100.protein.toFixed(2)), {
      shouldValidate: true,
    });
    setValue("carb_g_per_100g", parseFloat(labelPer100.carb.toFixed(2)), {
      shouldValidate: true,
    });
    setValue("fat_g_per_100g", parseFloat(labelPer100.fat.toFixed(2)), {
      shouldValidate: true,
    });
  }
  ...
  const ids = {
    ...
    fat: "cf-fat",
    carb: "cf-carb",
    unitName: "cf-unitName",
    kcalPerUnit: "cf-kcalUnit",
    proteinPerUnit: "cf-proteinUnit",
    carbPerUnit: "cf-carbUnit",
    fatPerUnit: "cf-fatUnit",
  } as const;
  ...
  return (
    <form onSubmit={handleSubmit(onCreateCustomFood)} className="space-y-4">
      <div className="border-b border-border-light pb-4 dark:border-border-dark">
        <Button
          type="button"
          className="btn-ghost btn-sm text-brand-primary dark:text-brand-primary hover:underline"
          onClick={() => setUseLabel((v) => !v)}
        >
          ...
          <div className="mt-2 p-3 border border-border-light rounded-md bg-surface-light dark:bg-border-dark/50 dark:border-border-dark space-y-3">
            <div className="grid grid-cols-2 gap-2">
              <div className="flex flex-col">
                <label htmlFor={ids.labelKcal} className="sr-only">
                  kcal per serving
                </label>
                <Input
                  id={ids.labelKcal}
                  type="number"
                  step={0.01}
                  placeholder="kcal / serv"
                  {...register("labelKcal", { valueAsNumber: true })}
                />
              </div>
              <div className="flex flex-col">
                <label htmlFor={ids.labelP} className="sr-only">
                  protein grams
                </label>
                <Input
                  id={ids.labelP}
                  ...
                  {...register("labelP", { valueAsNumber: true })}
                />
              </div>
              <div className="flex flex-col">
                <label htmlFor={ids.labelF} className="sr-only">
                  fat grams
                </label>
                <Input
                  id={ids.labelF}
                  ...
                  step={0.01}
                  placeholder="fat g"
                  {...register("labelF", { valueAsNumber: true })}
                />
              </div>
              <div className="flex flex-col">
                <label htmlFor={ids.labelC} className="sr-only">
                  carb grams
                </label>
                <Input
                  id={ids.labelC}
                  ...
                  step={0.01}
                  placeholder="carb g"
                  {...register("labelC", { valueAsNumber: true })}
                />
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="flex flex-col">
                <label htmlFor={ids.servAmt} className="sr-only">
                  Serving size
                </label>
                <Input
                  id={ids.servAmt}
                  ...
                  min={0}
                  step={0.01}
                  placeholder="Serv size"
                  {...register("servAmt", { valueAsNumber: true })}
                />
              </div>
              <div className="flex flex-col flex-1">
                <label htmlFor={ids.servUnit} className="sr-only">
                  Serving unit
                </label>
                <select
                  id={ids.servUnit}
            ...
            {servUnit !== "g" && (
              <div className="flex flex-col">
                <label htmlFor={ids.density} className="sr-only">
                  Density (g/ml)
                </label>
                <Input
                  id={ids.density}
            ...
            {labelPer100 && (
              <div className="flex flex-wrap items-center gap-3">
                <Button
                  ...
                  type="button"
                  onClick={applyConverterValues}
                >
                  Apply ↓
                </Button>
                <div className="text-xs text-text-muted dark:text-text-muted-dark">
                  <b>Per 100g:</b> {labelPer100.kcal.toFixed(0)}kcal,{" "}
                  {labelPer100.fat.toFixed(1)}F, {labelPer100.carb.toFixed(1)}C,{" "}
                  {labelPer100.protein.toFixed(1)}P
                </div>
              </div>
            )}
          </div>
      ...
      <div className="space-y-2">
        <h4 className="font-medium text-sm text-text dark:text-text-light">
          Create Food {unitName ? "(per unit)" : "(values per 100g)"}
        </h4>
        <div>
          ...
          <Input
            id={ids.description}
            placeholder="Description (e.g., Pop-Tarts)"
        ...
        <div>
          <label htmlFor={ids.brand} className="sr-only">
            Brand / store
          </label>
          <Input
            id={ids.brand}
            placeholder="Brand / store (optional)"
            {...register("brand_owner")}
          />
        </div>
        {!unitName && (
          <>
            <div className="flex flex-col">
              <label htmlFor={ids.kcal} className="sr-only">
                kcal
              </label>
              <Input
                id={ids.kcal}
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              <div className="flex flex-col">
                <label htmlFor={ids.fat} className="sr-only">
                  fat g
                </label>
                <Input
                  id={ids.fat}
              ...
              <div className="flex flex-col">
                <label htmlFor={ids.carb} className="sr-only">
                  carb g
                </label>
                <Input
                  id={ids.carb}
              ...
              <div className="flex flex-col">
                <label htmlFor={ids.protein} className="sr-only">
                  protein g
                </label>
                <Input
                  id={ids.protein}
        ...
        <div>
          <label htmlFor={ids.unitName} className="sr-only">
            Unit name
          </label>
          <Input
            id={ids.unitName}
            placeholder="Unit name (e.g., softgel)"
            {...register("unitName")}
          />
        </div>
        {unitName && (
          <>
            <div className="flex flex-col">
              <label htmlFor={ids.kcalPerUnit} className="sr-only">
                kcal per unit
              </label>
              <Input
                id={ids.kcalPerUnit}
            ...
            <div className="grid grid-cols-3 gap-2 mt-2">
              <div className="flex flex-col">
                <label htmlFor={ids.fatPerUnit} className="sr-only">
                  fat g per unit
                </label>
                <Input
                  id={ids.fatPerUnit}
              ...
              <div className="flex flex-col">
                <label htmlFor={ids.carbPerUnit} className="sr-only">
                  carb g per unit
                </label>
                <Input
                  id={ids.carbPerUnit}
              ...
              <div className="flex flex-col">
                <label htmlFor={ids.proteinPerUnit} className="sr-only">
                  protein g per unit
                </label>
                <Input
        ...
        <div className="flex justify-end pt-2">
          <Button
            ...
            disabled={isCreatingFood || !isValid}
          >
