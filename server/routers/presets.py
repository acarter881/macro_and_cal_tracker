...
from server.models import Food, FoodEntry, Meal, Preset, PresetItem
from server.utils import ensure_food_cached, get_or_create_meal
...
def list_presets(session: Session = Depends(get_session)):
    ...
    return {
        "items": [
            {"id": p.id, "name": p.name, "item_count": counts.get(p.id, 0)}
            for p in presets
        ]
    }
...
def get_preset_detail(preset_id: int, session: Session = Depends(get_session)):
    ...
    items = session.exec(
        select(PresetItem).where(PresetItem.preset_id == preset_id)
    ).all()
    fids = [it.fdc_id for it in items]
    foods = {
        f.fdc_id: f
        for f in session.exec(select(Food).where(Food.fdc_id.in_(fids))).all()
    }
    return {
        "id": p.id,
        "name": p.name,
        "items": [
            {
                ...
                "grams": it.grams,
                "description": (
                    foods.get(it.fdc_id).description
                    if foods.get(it.fdc_id)
                    else f"FDC {it.fdc_id}"
                ),
            }
            for it in items
        ],
...
def create_preset_from_meal(
    payload: PresetFromMeal, session: Session = Depends(get_session)
):
    date_str = payload.date.isoformat()
    m = session.exec(
        select(Meal).where(Meal.date == date_str, Meal.name == payload.meal_name)
    ).first()
    if not m:
    ...
    data = PresetCreate(
        name=payload.name,
        items=[PresetItemIn(fdc_id=e.fdc_id, grams=e.quantity_g) for e in entries],
    )
...
async def apply_preset(
    ...
    for it in items:
        ...
        max_order = (
            session.exec(
                select(func.max(FoodEntry.sort_order)).where(FoodEntry.meal_id == m.id)
            ).first()
            or 0
        )
        session.add(
...
def rename_preset(
    preset_id: int, payload: PresetRename, session: Session = Depends(get_session)
):
    p = session.get(Preset, preset_id)
    if not p:
        raise HTTPException(status_code=404, detail="Preset not found")
...
def update_preset_items(
    preset_id: int, payload: PresetItemsUpdate, session: Session = Depends(get_session)
):
    ...
    for it in payload.items:
        session.add(
            PresetItem(preset_id=preset_id, fdc_id=it.fdc_id, grams=float(it.grams))
        )
    session.commit()
    return {"ok": True}
