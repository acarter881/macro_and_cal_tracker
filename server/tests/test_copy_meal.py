...
def test_copy_meal_assigns_sort_order():
    ...
    with TestClient(app.app) as client:
        ...
        with Session(engine) as session:
            ...
            source_meal = Meal(
                date=date(2024, 1, 1).isoformat(), name="Meal 1", sort_order=1
            )
            dest_meal = Meal(
                date=date(2024, 1, 2).isoformat(), name="Meal 1", sort_order=1
            )
            session.add(food)
            ...
            session.add(
                FoodEntry(
                    meal_id=source_meal_id, fdc_id=1, quantity_g=100, sort_order=1
                )
            )
            session.add(
                FoodEntry(
                    meal_id=source_meal_id, fdc_id=1, quantity_g=150, sort_order=2
                )
            )
            session.add(
                FoodEntry(meal_id=dest_meal_id, fdc_id=1, quantity_g=50, sort_order=1)
            )
            session.commit()
